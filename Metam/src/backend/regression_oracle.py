...
class Oracle:
    ...
    def train(self, data, target_col):
        try:
            ...
            # Normalize MAE to a 0-1 scale (assuming MAE is non-negative)
            max_possible_mae = np.abs(y_test).max()
            normalized_score = 1 - (mae / max_possible_mae)

            return normalized_score

        except Exception as e:
            logger.error(f"Error in training regressor: {e}", exc_info=True)
            return None
